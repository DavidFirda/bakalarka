...
class TutorPolicyModel(pomdp_py.PolicyModel):
    def __init__(self, categories, weak_categories, initial_temperature=1, min_temperature=0.05, decay_rate=0.98):
    ...
    def sample(self, belief):
        ...
        for category, stats in self.category_performance.items():
            ...
            category_scores[category] = (1 - accuracy) * (3 if category in self.weak_categories else 1)  

        # **Prioritizujeme kategórie, kde je neistota najväčšia (blízko 50%)**
        for category, stats in self.category_performance.items():
            total = stats["correct"] + stats["incorrect"]
            if total > 0:
                p_correct = stats["correct"] / total
                entropy = -p_correct * np.log2(p_correct + 1e-6) - (1 - p_correct) * np.log2(1 - p_correct + 1e-6)
                category_scores[category] += entropy  # Pridáme váhu podľa neistoty
