...
class BayesianTutorPolicy(pomdp_py.PolicyModel):
    ...
    def update_performance(self, category, correct):
        """ Bayesovská aktualizácia pravdepodobnosti úspešnosti v kategórii. """
        if correct:
    ...
    def get_category_strength(self, category):
        """ Odhad úspešnosti kategórie na základe Beta distribúcie. """
        alpha = self.category_performance[category]["alpha"]
        beta = self.category_performance[category]["beta"]
        return alpha / (alpha + beta)  # Odhadovaná pravdepodobnosť správnej odpovede

    def identify_weak_categories(self):
        """ Nájde všetky slabé kategórie (pod stanovenou úspešnosťou). """
        return [cat for cat in self.categories if self.get_category_strength(cat) < self.weak_threshold]

    def sample(self, belief):
        """ Používa Thompson Sampling s viacerými slabými kategóriami. """
        ...
        if weak_categories:
            # Ak existuje viacero slabých kategórií, vyberieme z nich náhodne vážením
            weak_probs = {cat: sampled_probs[cat] for cat in weak_categories}
            best_category = max(weak_probs, key=weak_probs.get)
        else:
