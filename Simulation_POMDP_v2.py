...
from POMDP_v2 import TutorPOMDP
...
def run_pomdp_simulation(num_students, num_iterations):
    ...
    for category, probs in average_weak_probs.items():
...
def visualize_updated_results(results, student_question_counts, student_final_probs, average_probs, weak_categories):
...
def save_simulation_results_to_excel(results, student_question_counts, student_final_probs, average_probs, filename="simulation_POMDPv2_results.xlsx"):
    with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
        results_df = pd.DataFrame.from_dict(results)
        results_df.to_excel(writer, sheet_name="Overall Results")

        student_question_counts_df = pd.DataFrame.from_dict(student_question_counts, orient="index")
        student_question_counts_df.to_excel(writer, sheet_name="Student Question Counts")

        student_final_probs_df = pd.DataFrame.from_dict(student_final_probs, orient="index")
        student_final_probs_df.to_excel(writer, sheet_name="Student Final Probabilities")
        ...
        average_probs_df.to_excel(writer, sheet_name="Average Probabilities")
...
visualize_updated_results(pomdp_results, student_question_counts, student_final_probs, average_probs, weak_categories)
save_simulation_results_to_excel(pomdp_results, student_question_counts, student_final_probs, average_probs)